...
const ZenjsonErreurInternal: TErreurStore<TZenjsonErreurData> =
  createErreurStore<TZenjsonErreurData>();

export const ZenjsonErreur = ZenjsonErreurInternal.asReadonly;

export function throwCustomTypeNotFound(typeName: string): never {
  return ZenjsonErreurInternal.setAndThrow(
    `Unexpected: custom type "${typeName}" not found`,
    {
...
export function throwKeyNotFound(keyName: string): never {
  return ZenjsonErreurInternal.setAndThrow(`Key "${keyName}" not found`, {
    kind: "KeyNotFound",
    keyName,
  });
}
...
export function throwUnexpectedSpecialValue(value: unknown): never {
  return ZenjsonErreurInternal.setAndThrow(
    `Unexpected special number value ${String(value)}`,
    {
...
export function throwInvalidSerializedSpecialValue(value: string): never {
  return ZenjsonErreurInternal.setAndThrow(
    `Invalid serialized special number: ${value}`,
    {
...
export function throwDuplicatedCustomTypeName(typeName: string): never {
  return ZenjsonErreurInternal.setAndThrow(
    `Invalid custom type list: duplicated type name: "${typeName}"`,
    {
